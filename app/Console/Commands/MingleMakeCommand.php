...
class MingleMakeCommand extends GeneratorCommand
{
    ...
    protected function generateNewComponentContents($framework, $path, $basename): string
    {
        return match ($framework) {
            ...
            'vue' => str(
                $this->files->get(base_path('stubs/mingle.vue-component.stub'))
            )->replace('{{ $basename }}', $basename),

            'react' => str(
                $this->files->get(base_path('stubs/mingle.react-component.stub'))
            )->replace('{{ $basename }}', $basename),
    ...
    protected function buildJavascriptFile(string $basename, string $path, string $framework): string
    {
        ...
        $stub = str(
            $this->files->get(base_path('stubs/mingle.register.stub'))
        );
    ...
    {
        return base_path('stubs/mingle.livewire.stub');
    }
